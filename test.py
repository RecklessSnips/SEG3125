...
def process_input(history, message):
    user_text = ""
    files = message.get("files", [])  # ✅ 避免 `NoneType` 错误
    if files:
        for file in files:
            if file.endswith(".wav") or file.endswith(".mp3"):  # ✅ 处理语音文件
                transcribed_text = transcribe_audio(file)
                user_text = f"[🎤 语音输入]: {transcribed_text}"
                history.append((transcribed_text, ""))  # ✅ 确保 `history` 追加内容
            else:
                history.append((f"[📁 文件上传]: {file}", ""))
    
    if message.get("text"):  # ✅ 处理文本输入
        user_text = message["text"]
        history.append((user_text, ""))  # ✅ 保证历史记录正确追加

    return history, gr.MultimodalTextbox(value=None, interactive=True)  # ✅ 保持输入框不清空


# ✅ 发送请求到 Groq API
def groq_bot(history: list):
    """
    处理用户输入，并流式返回 Groq 生成的 AI 回复。
    """
    if not history or not history[-1][0]:  # ✅ 确保 `message` 存在
        return [("❌ 请输入有效内容", "")]

    message = history[-1][0]  # ✅ 获取最新的用户输入
    messages = [
        {
            "role": "system",
            "content": (
    ...
    # ✅ 加入历史对话
    for user_msg, ai_response in history[-3:]:  # 只保留最近 3 轮对话
        messages.append({"role": "user", "content": user_msg})
        if ai_response:
            messages.append({"role": "assistant", "content": ai_response})

    messages.append({"role": "user", "content": message})
    history.append((message, ""))  # ✅ 先存入用户输入，AI 回复留空
    ...
    for chunk in completion:
        ...
        full_response += content  # ✅ 逐步拼接 AI 回复
        history[-1] = (history[-1][0], full_response)  # ✅ 逐步更新 AI 回复
        yield history  # ✅ 逐步返回完整聊天记录
...
# conversation_history, _ = process_input(conversation_history, user_input)
def chat_with_bot_stream(user_input, audio, language):
    ...
    if len(conversation_history) == 1:
        conversation_history.insert(0, {
            "role": "system",
            "content": "You are a holiday planner. Provide structured responses about attractions to visit, hotels to stay at, and restaurants to eat at on holiday. You also speak 10+ languages and can respond in other languages if needed. " + language_prompt
        })
    else:
        conversation_history[0]["content"] += f" {language_prompt}"
    ...
    for i in range(1, len(conversation_history), 2):
        if i + 1 < len(conversation_history):
            ...
            if audio:
              #Get audio version of response
              tts = gTTS(bot_msg, lang='en')
